...
//finds a bucket in our hashmap
//Can create a bucket too
static inline DArray *Hashmap_find_bucket(Hashmap *map, void *key, 
		int create,
		uint32_t * hash_out)
{
	uint32_t hash = map->hash(key); //find the hash

	int bucket_n = hash % DEFAULT_NUMBER_OF_BUCKETS; //find the bucket it belongs to
...
{
	uint32_t hash = 0;
	//The hash fill be populated by find_bucket
		...
		DArray * bucket, void *key)
{
	...
	//if collision has occured or if it hasnt
